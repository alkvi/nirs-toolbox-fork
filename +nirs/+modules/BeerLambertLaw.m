classdef BeerLambertLaw < nirs.modules.AbstractModule
    ...
    methods
        ...
        function data = runThis( obj, data )
            for i = 1:numel(data)
                data(i) = local_runthis(obj,data(i));
            end
        end
    end
end

% I moved this out of the loop because now all the data is processed truly
% independently.  Previously, there was some carry-over of list variables
% if the probes differed between data scans.

function data = local_runthis(obj,data)
if(isa(data,'nirs.core.Data'))
    ...
    data.data  = reshape(d_chr,size(d_chr,1),size(d_chr,2)*size(d_chr,3));
    data.probe = p;
...
elseif(isa(data,'nirs.core.ChannelStats'))
    ...
    if(~ismember('source',data.probe.link.Properties.VariableNames) & ...
            ismember('ROI',data.probe.link.Properties.VariableNames))
        data=data.sorted({'ROI','type'});
    else
        % sort channels
    ...
    F=zeros(height(data.variables));
    N=cell(height(data.variables),1);
    N2=cell(height(data.probe.link),1);
    for j = 1:length(idx)
        lst = find(ismember(var,var(idx(j),:)));
        lst2 = find(ismember(var2(:,1:2),var(idx(j),1:2)));
        assert( length(lst) > 1 )

        lambda = data.variables.type(lst);
    ...
    data.beta=F*data.beta;
    data.covb=F*data.covb*F';
    data.variables.type=N;
    data.probe.link.type=N2;
